...
# CHANGE 1: Set theme to empty so it doesn't fight your custom prompt
ZSH_THEME=""  
...
# CHANGE 2: Remove the syntax/autosuggest plugins from here to stop the errors.  
# We load them manually at the bottom instead.
plugins=(git)
...
# CHANGE 3: This recreates your orange/green/dollar look.
# --- 4. THE "Chry@Chry" PROMPT ---
# %F{214} = Orange, %F{34} = Green, %F{white} = White
# %n@%m = user@host, %~ = directory, %D{%H:%M:%S} = time
...
alias explain=tldr
alias installaapp1="sudo apt install"
alias installaapp2="sudo snap install"
alias aggiornaeinstalla1="sudo apt update && sudo apt upgrade -y"
alias aggiornaeinstalla2="sudo snap refresh"

# --- IDA Pro Bottle Launcher ---
#In order to use this command u must have IDA pro installed on Wines on ur pc
...
# --- 8. UBUNTU PLUGIN FIX ---
# This is the "correct" way to load these on Ubuntu to avoid the "not found" errors
