...
scp -O "$CONF" "${TARGET_USER}@${TARGET_IP}:/tmp/zshconfig_arch.conf"
ssh "${TARGET_USER}@${TARGET_IP}" "mv /tmp/zshconfig_arch.conf ~/.zshrc && echo 'Config updated successfully.'"
